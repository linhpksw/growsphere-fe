    ...
    reducers: {
        wishlist_product: (state, { payload }: PayloadAction<CartProductType>) => {
            ...
            if (productIndex >= 0) {
                ...
                toast.success(`${capitalizedCategoryName} đã được thêm vào danh sách yêu thích`);
            }
        },
        remove_wishlist_product: (state, { payload }: PayloadAction<CartProductType>) => {
...
export const { wishlist_product, remove_wishlist_product, clear_wishlist, decrease_quantity } =
